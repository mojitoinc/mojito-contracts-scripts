...
async function main() {
  ...
  for(let i=0; i<resultSet.length; i++){
    ...
    console.log("ContractAddress",resultSet[i][0],"Token ID",resultSet[i][1],"Recipient Address",resultSet[i][2],"Percentage",percentage); 
    const receipt = await contract.setTokenRoyalty(resultSet[i][0],resultSet[i][1],[resultSet[i][2]],[percentage]);
