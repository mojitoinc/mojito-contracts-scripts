...
abstract contract RedeemSetBase is AdminControl, IRedeemSetBase {
    ...
    function configureRedemptionSet(RedemptionItem[] memory redemptionSet)
        internal
    {
        for (uint256 i = 0; i < redemptionSet.length; i++) {
            RedemptionItem memory redemptionItem = redemptionSet[i];
            require(
                redemptionItem.minTokenId <= redemptionItem.maxTokenId,
                "Redeem: min must be less or equal to max"
            );
            _redemptionSet.push(redemptionItem);
            emit RedeemSetApprovedTokenRange(
                redemptionItem.tokenAddress,
                redemptionItem.minTokenId,
                redemptionItem.maxTokenId
            );
        }
    }
    ...
    {
        return
            interfaceId == type(IRedeemSetBase).interfaceId ||
     ...
     */
    function getRedemptionSet()
        external
        view
        override
        returns (RedemptionItem[] memory)
    {
     ...
     */
    function _validateCompleteSet(
        address[] memory contracts,
        uint256[] memory tokenIds
    ) internal view virtual returns (bool) {
        require(
            _redemptionSet.length == tokenIds.length,
            "Incorrect number of NFTs being redeemed"
        );
        // Check complete set
        bool[] memory completions = new bool[](_redemptionSet.length);
        for (uint256 i = 0; i < contracts.length; i++) {
            for (uint256 j = 0; j < _redemptionSet.length; j++) {
                RedemptionItem memory redemptionItem = _redemptionSet[j];
                if (
                    contracts[i] == redemptionItem.tokenAddress &&
                    tokenIds[i] >= redemptionItem.minTokenId &&
                    tokenIds[i] <= redemptionItem.maxTokenId
                ) {
