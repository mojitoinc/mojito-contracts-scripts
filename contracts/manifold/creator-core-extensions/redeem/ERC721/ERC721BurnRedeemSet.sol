 ...
 */
contract ERC721BurnRedeemSet is
    ReentrancyGuard,
    ERC721RedeemSetBase,
    IERC721BurnRedeemSet,
    IERC1155Receiver
{
    ...
    ) ERC721RedeemSetBase(creator, redemptionSet, redemptionMax) {}
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721RedeemSetBase, IERC165)
        returns (bool)
    {
        return
            interfaceId == type(IERC721BurnRedeemSet).interfaceId ||
    ...
    function redeemERC721(
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            ...
            if (
                !IERC721(contracts[i]).isApprovedForAll(
                    msg.sender,
                    address(this)
                )
            ) {
