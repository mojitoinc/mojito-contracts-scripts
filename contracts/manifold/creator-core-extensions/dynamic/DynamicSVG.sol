...
import "../../creator-core/extensions/ERC721/IERC721CreatorExtensionApproveTransfer.sol";

import "../libraries/ABDKMath64x64.sol";
import "../libraries/SingleCreatorExtension.sol";

contract DynamicSVG is
    ...
    IERC721CreatorExtensionApproveTransfer
{
    ...
    string private constant _HUE1_TAG = "<HUE1>";
    string private constant _SATURATION1_TAG = "<SATURATION1>";
    string private constant _LIGHTNESS1_TAG = "<LIGHTNESS1>";
    string private constant _HUE2_TAG = "<HUE2>";
    string private constant _SATURATION2_TAG = "<SATURATION2>";
    string private constant _LIGHTNESS2_TAG = "<LIGHTNESS2>";
    ...
    constructor(address creator) ERC721SingleCreatorExtension(creator) {
        ...
        _imageParts.push(
            "<defs><linearGradient id='g' x1='0%' x2='0%' y1='1%' y2='100%'><stop offset='0%' stop-color='hsl("
        );
        ...
        _imageParts.push(
            "%)' /><stop offset='100%' stop-color='hsl(0,0%,15%)' /></linearGradient></defs>"
        );
        _imageParts.push(
            "<g><rect x='-0.5' y='-0.5' width='1' height='1' fill='hsl(0,0%,15%)' /><circle cx='0' cy='0' r='"
        );
        _imageParts.push(_RADIUS_TAG);
        _imageParts.push(
            "' fill='url(#g)'><animateTransform attributeName='transform' type='rotate' from='0' to='360' dur='60s' repeatCount='indefinite' /></circle></g>"
        );
        ...
        returns (string memory)
    {
        ...
        if (_completionTimestamp > block.timestamp) {
            completion = ABDKMath64x64.divu(
                _completionTimestamp - block.timestamp,
                31536000
            );
        }
        int128 distance = completion.pow(3).mul(ABDKMath64x64.div(48, 100)).add(
            ABDKMath64x64.div(2, 100)
        ) + 1;

        //x^y = 2^(y*log_2(x))
        int128 c1curve = ABDKMath64x64.exp_2(
            ABDKMath64x64.div(75, 100).mul(ABDKMath64x64.log_2(completion))
        );
        int128 c2curve = ABDKMath64x64.exp_2(
            ABDKMath64x64.div(125, 100).mul(ABDKMath64x64.log_2(completion))
        );
        int128 w1value = ABDKMath64x64
            .fromUInt(
                uint256(uint160(IERC721(creator).ownerOf(tokenId)) & 0xFF)
            )
            .div(ABDKMath64x64.fromUInt(255));
        int128 w2value = ABDKMath64x64
            .fromUInt(
                uint256(
                    (uint160(IERC721(creator).ownerOf(tokenId)) >> 8) & 0xFF
                )
            )
            .div(ABDKMath64x64.fromUInt(255));

        int128 randHue = ABDKMath64x64.mul(
            w1value,
            ABDKMath64x64.fromUInt(360)
        );
        int128 randOffset = ABDKMath64x64
            .mul(w2value, ABDKMath64x64.fromUInt(180))
            .add(ABDKMath64x64.fromUInt(360))
            .sub(ABDKMath64x64.fromUInt(90));

        return
            string(
                abi.encodePacked(
                    'data:application/json;utf8,{"name":"Dynamic", "description":"Days passed: ',
                    ((block.timestamp - _creationTimestamp) / 86400).toString(),
                    '", "image":"',
                    _generateImage(
                        distance,
                        completion,
    ...
    function _radiusString(int128 distance)
        private
        pure
        returns (string memory)
    {
    ...
    function _hue1string(int128 completion, int128 randHue)
        private
        pure
        returns (string memory)
    {
        int128 hueValue = randHue.add(
            completion.mul(ABDKMath64x64.fromUInt(60))
        );
        uint256 decimal4 = (hueValue & 0xFFFFFFFFFFFFFFFF).mulu(10000);
        return
            string(
                abi.encodePacked(
                    _toString(hueValue.toInt() % 360),
                    ".",
    ...
    function _saturation1String(int128 c1curve)
        private
        pure
        returns (string memory)
    {
    ...
    function _lightness1String(int128 c1curve)
        private
        pure
        returns (string memory)
    {
        return
            _floatToString(
                c1curve.mul(ABDKMath64x64.fromUInt(70)) +
                    ABDKMath64x64.fromUInt(15)
            );
    }

    function _hue2string(
        int128 completion,
        int128 randHue,
        int128 randOffset
    ) private pure returns (string memory) {
        int128 hueValue = randHue.add(
            completion.mul(ABDKMath64x64.fromUInt(60)).add(randOffset)
        );
        uint256 decimal4 = (hueValue & 0xFFFFFFFFFFFFFFFF).mulu(10000);
        return
            string(
    ...
    function _saturation2String(int128 c2curve)
        private
        pure
        returns (string memory)
    {
    ...
    function _lightness2String(int128 c2curve)
        private
        pure
        returns (string memory)
    {
        return
            _floatToString(
                c2curve.mul(ABDKMath64x64.fromUInt(35)) +
                    ABDKMath64x64.fromUInt(15)
            );
    }
