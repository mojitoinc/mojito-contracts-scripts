...
contract TimeToken is DynamicArweaveHash {
    ...
    function _getImageHash(uint256)
        internal
        view
        override
        returns (string memory)
    {
        return
            imageArweaveHashes[
                ((block.timestamp - _creationTimestamp) / _cadence) %
                    imageArweaveHashes.length
            ];
    }

    function _getAnimationHash(uint256)
        internal
        view
        override
        returns (string memory)
    {
        return
            animationArweaveHashes[
                ((block.timestamp - _creationTimestamp) / _cadence) %
                    animationArweaveHashes.length
            ];
    }
}
