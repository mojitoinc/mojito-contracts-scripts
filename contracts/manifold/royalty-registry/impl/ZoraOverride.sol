...
contract ZoraOverride is IZoraOverride, ERC165 {
    ...
    {
        return
            interfaceId == type(IZoraOverride).interfaceId ||
    ...
    {
        IZoraMarket.ZoraBidShares memory bidShares = IZoraMarket(
            IZoraMedia(media).marketContract()
        ...
        if (bidShares.creator.value != 0) {
            receivers[currentIndex] = payable(
                IZoraMedia(media).tokenCreators(tokenId)
            );
        ...
        if (bidShares.owner.value != 0) {
            receivers[currentIndex] = payable(
                IZoraMedia(media).ownerOf(tokenId)
            );
